...
class AiBrain(private val learner: OnlineLearner) {
    fun decide(state: GameState): AiAction {
        ...
        if (nearestEnemy != null) {
            ...
            if (d < state.width * 0.34f) return AiAction(AiAction.Type.ATTACK, nearestEnemy.center.x, nearestEnemy.center.y, 80L, learner.score("attack"))
...
class OnlineLearner {
    ...
    fun setValue(action: String, value: Float) { values[action] = value.coerceIn(-10f, 10f) }
    fun setDodgeSide(left: Boolean) { preferLeftDodge = left }
